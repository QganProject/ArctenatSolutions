// Hero text animation - single DOM update, requestAnimationFrame, cached spans
const animTextElements = document.querySelectorAll(".anim-text");
const INTERVAL_MS = 30;

animTextElements.forEach((item) => {
...
// Hero cards: scroll to What we do and highlight the clicked service card
const SERVICE_IDS = ["services-cloud", "services-web", "services-power"];
const HIGHLIGHT_DURATION = 4000;

function highlightServiceFromHash() {
	const hash = window.location.hash.slice(1);
	if (!SERVICE_IDS.includes(hash)) return;
	const card = document.getElementById(hash);
	const section = document.getElementById("services");
	if (!card || !section) return;
	document.querySelectorAll(".service-card.service-card-highlight").forEach((el) => el.classList.remove("service-card-highlight"));
...
if (SERVICE_IDS.some((id) => window.location.hash === "#" + id)) {
	if (document.readyState === "loading") {
		document.addEventListener("DOMContentLoaded", highlightServiceFromHash);
	} else {
...
// Blue center line: grows as user scrolls from hero to About Us title
(function () {
	var line = document.getElementById("hero-to-about-line");
	var aboutSection = document.getElementById("about-us");
	if (!line || !aboutSection) return;

	function updateLine() {
		var slideshow = document.getElementById("slideshow");
		var heroBottom = slideshow ? slideshow.getBoundingClientRect().bottom : window.innerHeight;
		var aboutTop = aboutSection.getBoundingClientRect().top;
		var gap = aboutTop - heroBottom;
		if (gap <= 0) {
			line.style.top = aboutTop + "px";
			line.style.height = heroBottom - aboutTop + "px";
		} else {
			line.style.top = heroBottom + "px";
...
// Header: transparent at top, blue background when scrolled
(function () {
...
(function () {
	...
	function scrollToSectionCentered(el) {
		...
		const viewportCenterBelowHeader = headerHeight + (window.innerHeight - headerHeight) / 2;
		const targetScroll = sectionCenter - viewportCenterBelowHeader;
		const maxScroll = document.documentElement.scrollHeight - window.innerHeight;
		const clampedScroll = Math.max(0, Math.min(targetScroll, maxScroll));
		window.scrollTo({ top: clampedScroll, behavior: "smooth" });
	}
